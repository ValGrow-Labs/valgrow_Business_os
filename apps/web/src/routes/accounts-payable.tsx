...
import { useSupplierBalances, useSupplierAging, useSupplierStatement } from "@/hooks/queries/useApSubledger";
import { Calendar, UserCheck, FileText } from "lucide-react";

export const Route = createFileRoute("/accounts-payable")({
  head: () => ({
    meta: [{ title: "Accounts Payable · ValGrow Business OS" }],
  }),
  component: AccountsPayablePage,
});

function AccountsPayablePage() {
  const { data: balances, isLoading: balLoading } = useSupplierBalances();
  const { data: aging } = useSupplierAging();

  const [selectedSupplierId, setSelectedSupplierId] = useState<string | null>(null);
  const { data: statement } = useSupplierStatement(selectedSupplierId || "", undefined, undefined);

  return (
    <AppShell>
      <PageHeader
        title="Accounts Payable (AP)"
        description="Supplier vendor payables, aging buckets (1-90+ days), and account ledgers."
        eyebrow="Payables Management"
      />

      <Tabs defaultValue="balances">
        <TabsList className="mb-4">
          <TabsTrigger value="balances" className="gap-2">
            <UserCheck className="h-4 w-4" /> Supplier Balances
          </TabsTrigger>
          <TabsTrigger value="aging" className="gap-2">
            <Calendar className="h-4 w-4" /> AP Aging Analysis
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="balances">
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b bg-muted/50 text-xs font-semibold uppercase text-muted-foreground">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">Total Payable Balance</th>
              ...
              <tbody className="divide-y">
                {balLoading ? (
                  ...
                  balances?.map((b) => (
                    <tr key={b.supplier.id} className="hover:bg-accent/5">
                      <td className="px-4 py-3 font-mono font-bold text-primary">{b.supplier.code}</td>
                      <td className="px-4 py-3 font-medium">{b.supplier.name}</td>
                      <td className="px-4 py-3 text-muted-foreground">{b.supplier.email || b.supplier.phone || "-"}</td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          onClick={() => setSelectedSupplierId(b.supplier.id)}
                        >
          ...
          {selectedSupplierId && statement && (
            <Card className="mt-6">
              <CardHeader>
                <CardTitle>Supplier Statement — {statement.supplier.name}</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-left">
                    <thead className="border-b bg-muted/50 text-xs uppercase font-semibold text-muted-foreground">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-right">Debit (Payment)</th>
                        <th className="px-4 py-2 text-right">Credit (Invoice)</th>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y">
                    {aging.suppliers.map((s) => (
                      <tr key={s.supplier.id} className="hover:bg-accent/5">
